...
$RADIOMICS_CSV = '/Volumes/YANA 1TB/cbis-ddsm-r/data/CBIS-DDSM-R/csv/radiomics_test.csv';
  ...
  <div class="main-container">
    <header class="header">
      ...
      <div class="quick-guide">
        ...
        <ul>
          <li><strong>Step 1:</strong> Upload mammogram image
          </li>
    ...
    <div class="left-column">
      ...
      <div class="sticky-controls">
        <button type="button" id="toggle-sticky-btn" class="btn btn-secondary btn-small" title="Toggle Sticky Panel"
          aria-pressed="false">
          ...
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"
            class="unpin-icon" style="display:none;">
            <path
      ...
      <div class="step-card">
        ...
        <form id="image-upload-form" method="post" enctype="multipart/form-data">
          ...
          <div id="overlay-preview-wrapper" style="display:none; margin-top:1rem; text-align:center;">

            <h4 style="margin-bottom:0.5rem;">Overlay</h4>
            <img id="overlay-preview-img" src="" style="max-width:100%; border-radius:8px;">
          </div>
      ...
      <div class="step-card text-center">
        ...
        <button class="btn" type="submit" id="submit-btn" disabled form="image-upload-form"> <span id="btn-text">Run
            Prediction</span>
      ...
      <div class="step-card" id="history-card">
        <div class="step-header">
          ...
          <button type="button" class="btn btn-secondary btn-small" id="clear-history-btn" title="Clear All History"
            style="display: none;">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
    ...
    <div class="right-column">
      ...
      <div id="results-placeholder" style="display: block;">
        <div class="step-card placeholder-card single-placeholder">
          ...
          <div class="placeholder-content"> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
      ...
      <div class="results-container animate-slide-up" id="results-container" style="display:none;">
        <div class="step-card">
          <div class="step-header">
            ...
            <div class="header-buttons">
              <button type="button" class="btn btn-print" id="print-btn" title="Print Report"> <svg
                  ...
                  stroke-width="1.8" width="20" height="20" aria-hidden="true">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M7 9V5a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v4" />
                  <path stroke-linecap="round" stroke-linejoin="round"
          ...
          <div class="results-grid" id="results-grid">
            ...
            <div class="step-card prediction-card animate-slide-up full-bleed-card" id="prediction-card-content">

              <div class="step-header">
                <div class="step-header-left">
                  <h2 style="padding-left:0; margin:0;">Final Prediction <span class="pill pill-rule">Rule-based</span>
                  </h2>
                </div>
              ...
              <div class="card-content prediction-score-container">
                <div class="prediction-text-wrapper">
                  <span class="prediction-indicator"></span>
                  <span class="prediction-score-text" data-field="final_prediction">—</span>
                </div>
              </div>

              <div id="lesion-description-block" style="display:none;">
                <div class="lesion-info-panel">
                  <h3 class="lesion-info-header">
                  ...
                  <div id="lesion-description-content">
                  </div>
                </div>
              </div>
            ...
            <div class="step-card animate-slide-up" id="all-features-tables-card"
              style="animation-delay:.40s; grid-column: 1 / -1; margin-bottom: 2rem;">
              <div class="step-header">
                <div class="step-header-left">
                  <h2>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                ...
                <span class="tooltip-icon">i<span class="tooltip-content">All contributing features from your Python
                    script's `all_detected_delta` output. Sorted from most Benign-leaning (positive) to most
                    Malignant-leaning (negative).</span></span>
                <button type="button" class="maximize-card-btn" title="Maximize">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                    viewBox="0 0 16 16">
                    <path
            ...
            <div class="step-card animate-slide-up" id="all-features-charts-card"
              style="animation-delay:.50s; grid-column: 1 / -1;">
              <div class="step-header">
                <div class="step-header-left">
                  <h2>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                ...
                <span class="tooltip-icon">i<span class="tooltip-content">Visual representation of all contributing
                    features from your Python script's `all_detected_delta` output.</span></span>
                <button type="button" class="maximize-card-btn" title="Maximize">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                    viewBox="0 0 16 16">
                    <path
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function safe(fn) {
      try {
        ...
        console.warn('safe helper caught', e);
        return null;
      }
    }
    ...
    function showError(msg) {
      ...
      errorContainer.innerHTML =
        `<div class="step-card error-card animate-slide-up"><strong>Error:</strong> ${msg}</div>`;
    }
    ...
    function addResultToHistory(payload) {
      ...
      try {
        ...
        const item = {
          ...
          overlayPath: payload.overlay || null, // <--- ADDED THIS LINE
        ...
        console.error('addResultToHistory err', e);
      }
    }
    ...
    function renderHistory() {
      ...
      historyList.innerHTML = history.map(item => {
        ...
        const predClass = (String(pred || '').toLowerCase().startsWith('mal')) ? 'history-item-malignant' :
          'history-item-benign';
    ...
    function renderToCanvas(file) {
      return new Promise((res, rej) => {
        const isTiff = file.type === 'image/tiff' || file.name.toLowerCase().endsWith('.tif') || file.name
      ...
      activeCharts[canvasId] = new Chart(cv.getContext('2d'), {
        ...
        options: {
          ...
          scales: {
            x: {
              grid: {
                color: getComputedStyle(document.documentElement).getPropertyValue('--border-color') || '#eee'
              },
              ticks: {
                color: getComputedStyle(document.documentElement).getPropertyValue('--text-dark') || '#333',
          ...
          plugins: {
            ...
            tooltip: {
              callbacks: {
                label: ctx => `${valueLabel}: ${Number(ctx.parsed.x).toFixed(6)}`
              }
            }
          }
        }
    ...
    function renderAllFeaturesChart(canvasId, zDataSorted) {
      ...
      const colors = zDataSorted.map(d => Number(d.z) >= 0 ? (getComputedStyle(document.documentElement)
        .getPropertyValue('--pastel-benign') || 'rgba(132,204,145,0.7)') : (getComputedStyle(document
        .documentElement).getPropertyValue('--pastel-malignant') || 'rgba(252,165,165,0.7)'));
      const chartHeight = Math.max(400, zDataSorted.length * 18);
      cv.parentElement && (cv.parentElement.style.height = `${chartHeight}px`);
      const tableScroll = $('all-features-table-scroll');
      ...
      activeCharts[canvasId] = new Chart(cv.getContext('2d'), {
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              callbacks: {
                label: ctx => `Contribution: ${Number(ctx.parsed.x).toFixed(3)}`
              }
            }
          }
        }
    ...
    function renderColorCodedTable(tbodyId, featuresData, valueType = 'Value') {
      ...
      const rows = featuresData.map(([name, value]) => {
        ...
        const formattedValue = Number.isFinite(val) ? (valueType === 'Z-Score' ? val.toFixed(4) : val.toFixed(
          6)) : String(value);
    ...
    function downloadCSV(rd) {
      ...
      if (rd.probabilities) Object.entries(rd.probabilities).forEach(([k, v]) => c +=
        `Probability,${e(k)},${e(v)}\r\n`);

      c +=
        `Summary,lesion_subtype,${e(rd.lesion_subtype?.category ? `${rd.lesion_subtype.category} (${Object.values(rd.lesion_subtype.details || {}).join(', ')})` : rd.abnormality_type ?? '')}\r\n`;
      ...
      if (rd.abnormality_scores) Object.entries(rd.abnormality_scores).forEach(([k, v]) => c +=
        `Abnormality Score,${e(PRETTY_NAMES[k] || k)},${e(v)}\r\n`);
      ...
      if (rd.explanation?.class) c +=
        `Explanation,class,${e(Array.isArray(rd.explanation.class) ? rd.explanation.class.join('; ') : rd.explanation.class)}\r\n`;
      if (rd.explanation?.abnormality_summary) c +=
        `Explanation,abnormality_summary,${e(rd.explanation.abnormality_summary)}\r\n`;

      if (Array.isArray(rd.top_malignant_delta)) rd.top_malignant_delta.forEach(([n, v]) => c +=
        `Top Malignant Delta,${e(PRETTY_NAMES[n] || n)},${e(v)}\r\n`);
      if (Array.isArray(rd.top_benign_delta)) rd.top_benign_delta.forEach(([n, v]) => c +=
        `Top Benign Delta,${e(PRETTY_NAMES[n] || n)},${e(v)}\r\n`);
      if (Array.isArray(rd.all_detected_delta)) rd.all_detected_delta.forEach(([n, v]) => c +=
        `All Detected Delta,${e(PRETTY_NAMES[n] || n)},${e(v)}\r\n`);

      const zscoresToExclude = ["crop_x1", "crop_y1", "crop_ok"];
      if (rd.zscores) Object.keys(rd.zscores || {}).sort().filter(k => !zscoresToExclude.includes(k)).forEach(k =>
        c += `Z-Score,${e(PRETTY_NAMES[k] || k)},${e(rd.zscores[k])}\r\n`);
    ...
    function buildCSVPreviewHTML(rd, l = 35) {
      ...
      if (rd.probabilities) Object.entries(rd.probabilities).forEach(([k, v]) => rs.push(["Probability", k, String(
        v ?? "—")]));
      ...
      if (rd.abnormality_scores) Object.entries(rd.abnormality_scores).forEach(([k, v]) => rs.push([
        "Abnormality Score", PRETTY_NAMES[k] || k, String(v ?? "—")
      ]));
      if (rd.background_tissue) {
        rs.push(["Background", "code", String(rd.background_tissue.code ?? "—")]);
        rs.push(["Background", "text", String(rd.background_tissue.text ?? "—")]);
        rs.push(["Background", "explain", String(rd.background_tissue.explain ?? "—")]);
      }
      if (rd.explanation?.class) rs.push(["Explanation", "class", String(Array.isArray(rd.explanation.class) ? rd
        .explanation.class.join("; ") : rd.explanation.class)]);
      if (rd.explanation?.abnormality_summary) rs.push(["Explanation", "abnormality_summary", String(rd.explanation
        .abnormality_summary)]);

      if (Array.isArray(rd.top_malignant_delta)) rd.top_malignant_delta.forEach(([n, v]) => rs.push([
        "Top Malignant Delta", PRETTY_NAMES[n] || n, String(v ?? "—")
      ]));
      if (Array.isArray(rd.top_benign_delta)) rd.top_benign_delta.forEach(([n, v]) => rs.push(["Top Benign Delta",
        PRETTY_NAMES[n] || n, String(v ?? "—")
      ]));
      if (Array.isArray(rd.all_detected_delta)) rd.all_detected_delta.forEach(([n, v]) => rs.push([
        "All Detected Delta", PRETTY_NAMES[n] || n, String(v ?? "—")
      ]));

      const zscoresToExclude = ["crop_x1", "crop_y1", "crop_ok"];
      if (rd.zscores) Object.keys(rd.zscores || {}).sort().filter(k => !zscoresToExclude.includes(k)).forEach(k =>
        rs.push(["Z-Score", PRETTY_NAMES[k] || k, String(rd.zscores[k] ?? "—")]));

      const lim = rs.slice(0, l);
      let t =
        '<div class="table-wrapper-scroll"><table class="data-table"><thead><tr><th>Category</th><th>Parameter</th><th>Value</th></tr></thead><tbody>';
      lim.forEach(r => t +=
        `<tr><td>${escapeHTML(r[0])}</td><td>${escapeHTML(r[1])}</td><td>${escapeHTML(r[2])}</td></tr>`);
      t += '</tbody></table></div>';
      t +=
        `<div class="modal__actions" style="display:flex;gap:.5rem;justify-content:flex-end;margin-top:.75rem;"><button type="button" class="btn" id="csv-download-confirm">Download All ${rs.length} Rows</button><button type="button" class="btn btn-secondary" id="csv-preview-close">Close</button></div><p class="file-meta" style="margin-top:.5rem; text-align:right;">Showing first ${lim.length} of ${rs.length} rows.</p>`;
      return t;
    }
    ...
    function showContentInModal(title, contentHtml, cardId = null) {
      ...
      // Small delay to ensure DOM nodes are present before rendering charts
      requestAnimationFrame(() => {
        if (!window.__PREDICT__?.result) return;

        // Determine which charts need to be rendered based on the card ID
        const ids = [];
        if (cardId === 'all-features-charts-card') ids.push('all-features-chart');
        if (cardId === 'feature-summary-card-content') ids.push('fsummary-malignant-chart',
          'fsummary-benign-chart');
        if (cardId === 'explanation-card-content') ids.push('abnormality-chart');

        ids.forEach(id => {
          // 1. Find the canvas specifically inside the modal body
          const canvas = cardModalBody.querySelector(`#${id}`);
          if (!canvas) return;

          // 2. CRITICAL FIX: Rename the ID to avoid collision with the background chart
          const newId = id + '_modal';
          canvas.id = newId;

          // 3. Find the original chart instance
          const original = activeCharts[id];
          if (original && canvas.getContext) {
            try {
              const ctx = canvas.getContext('2d');

              // 4. Clone config using direct reference (preserves formatters/functions)
              // JSON.stringify/parse would strip out the axis number formatting
              const cfg = {
                type: original.config.type,
                data: original.config.data,
                options: original.config.options
              };

              // 5. Create new chart on the modal canvas
    ...
    function displayResults(resultData, payload) {
      ...
      const accentSuccess = getComputedStyle(document.documentElement).getPropertyValue('--accent-success') ||
        'rgba(46,204,113,0.7)';
      const accentWarning = getComputedStyle(document.documentElement).getPropertyValue('--accent-warning') ||
        'rgba(231,76,60,0.7)';
      ...
      if (predEl) {
        ...
        const pC = predEl.closest('.prediction-card');
        if (pC) pC.className =
          `step-card prediction-card animate-slide-up prediction-${predClass.replace(/\s+/g,'-')}`;
      }
      if (indEl) {
        const bSVG =
          `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="${accentSuccess}"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>`;
        const mSVG =
          `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="${accentWarning}"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" /></svg>`;
      ...
      (function renderExplanations() {
        ...
        []).filter(e => !(e || '').includes("Mahalanobis ratio:")).map(e => escapeHTML(e));
        ...
        function decorateMath(s) {
          if (!s) return '';
          return `<span class="math-inline">${s.replace(/<=/g, '≤').replace(/->/g, '→').replace(/\*/g, '·')}</span>`;
        }
        ...
        function metricsToChips(summary) {
          ...
          let m;
          while ((m = re.exec(summary)) !== null) c.push(
            `<span class="metric-chip"><span class="k">${m[1].trim()}</span><span class="v">${Number(m[2]).toFixed(2)}</span></span>`
          );
          return c.join('');
        }
        const classHTML = (cExp && cExp !== '—') ?
          `<div class="explain-section"><div class="explain-body">${decorateMath(cExp || '')}</div></div>` : '';
        const metricsHTML = metricsToChips(aSumm || '');
        const badgesHTML = `<div class="badge-row"></div>`;
        const summaryHTML =
          `<div class="explain-section"><div class="explain-title"><span class="dot"></span>Abnormality Summary</div><div class="explain-body">${metricsHTML || ''}${badgesHTML}</div></div>`;
        ...
        activeCharts['abnormality-chart'] = new Chart(abnCanvas.getContext('2d'), {
          ...
          data: {
            ...
            datasets: [{
              label: 'Score',
              data: abnVals,
              backgroundColor: abnVals.map((_, i) => ['rgba(99,179,237,0.7)', 'rgba(132,204,145,0.7)',
                'rgba(250,202,154,0.7)'
              ][i % 3]),
              borderWidth: 0,
              borderRadius: 4
            }]
      ...
      // --- Render lesion_metadata and lesion_narrative ---
      (function renderLesionDescription() {
        ...
        // If absolutely no data, hide the block
        if (!narrative && Object.keys(meta).length === 0 && !birads) {
          descRoot.style.display = 'none';
          return;
        }

        let html = '';

        // 1. Narrative (Main description)
        if (narrative) {
          html += `<div class="lesion-narrative-text">${escapeHTML(narrative)}</div>`;
        }

        // 2. Metadata Grid (Clean Layout)
        if (Object.keys(meta).length > 0) {
          html += `<div class="lesion-data-grid">`;

          // Define preferred order of keys
          const preferredOrder = ['abnormality_type', 'calc_type', 'calc_distribution', 'mass_shape',
            'mass_margins', 'assessment', 'subtlety'
          ];

          // Render preferred keys first
          preferredOrder.forEach(key => {
            if (meta[key] !== undefined && meta[key] !== null) {
              html += `
                 <div class="lesion-data-item">
                    <span class="lesion-data-label">${escapeHTML(key.replace(/_/g, ' '))}</span>
                    <span class="lesion-data-value">${escapeHTML(String(meta[key]))}</span>
                 </div>`;
              // Mark as null so we don't print it again in the loop below
              meta[key] = null;
            }
          });
          ...
          Object.entries(meta).forEach(([k, v]) => {
            ...
            html += `
             <div class="lesion-data-item">
                <span class="lesion-data-label">${escapeHTML(k.replace(/_/g, ' '))}</span>
                <span class="lesion-data-value">${escapeHTML(String(v))}</span>
             </div>`;
          });
        ...
        // 3. BI-RADS Badge (if separate)
        if (birads) {
          html +=
            `<div style="margin-top:2rem;"><div class="lesion-birads-box"><strong style="color:var(--accent-glow);">Suggested BI-RADS:</strong> <span style="font-weight:700; margin-left:5px;">${escapeHTML(birads)}</span></div></div>`;
        }
      ...
      if (document.getElementById(fsummaryMalBodyId)) {
        ...
        if (document.getElementById(fsummaryMalBodyId)) document.getElementById(fsummaryMalBodyId).innerHTML =
      ...
      if (document.getElementById(fsummaryBenBodyId)) {
        ...
        if (document.getElementById(fsummaryBenBodyId)) document.getElementById(fsummaryBenBodyId).innerHTML =
          '<tr><td colspan="2">No features found.</td></tr>';
      ...
      if (document.getElementById(allFeaturesTableBodyId) && allDetectedData.length > 0) {
        ...
        const allDataForChart = allDataSorted.map(([k, v]) => ({
          key: k,
          label: PRETTY_NAMES[k] || k,
          z: v
        }));
        if (document.getElementById('all-features-chart')) renderAllFeaturesChart('all-features-chart',
          allDataForChart);
      } else {
        // Table fallback
        if (document.getElementById(allFeaturesTableBodyId)) {
          document.getElementById(allFeaturesTableBodyId).innerHTML =
            '<tr><td colspan="2">No features found.</td></tr>';
        }
        // Chart fallback
      ...
      const topContribs = Array.isArray(resultData.top_feature_contributions) ? resultData
...
`;
      ...
      saveState({
        result: resultData,
        imagePath: payload?.image || payload?.preview || null,
        filename: fileInput?.files?. [0]?.name || null
      });
        ...
        if (previewWrapper) {
          ...
          img.onload = () => {
            try {
              ...
              if (nE) {
                nE.textContent = (fileInput?.files?. [0]?.name || (payload.image || '').split('/').pop() ||
                  'image');
          ...
          img.onerror = () => {
            console.warn('Could not load preview image:', payload.preview);
          };
          let src = payload.preview;
          if (src && !src.startsWith('http') && window.location) src = (window.location.origin || '') + '/' + src
            .replace(/^\/+/, '');
          img.src = src;
        }
      } else {
    ...
    function handleFileSelect(file) {
      if (!file) {
        if (fileInput && fileInput.files && fileInput.files.length) file = fileInput.files[0];
        else return;
      }
      ...
      if (ext === 'dcm') {
        ...
        if (previewWrapper) {
          previewWrapper.innerHTML =
            '<div class="loading-preview">DICOM detected — preview will load after server processing...</div>';
    ...
    if (clearBtn) {
      clearBtn.addEventListener('click', () => {
        ...
        // 1. Clear Main Preview
        if (previewWrapper) {
          previewWrapper.style.display = 'none';
          previewWrapper.removeAttribute('data-full-image');
          previewWrapper.innerHTML = ''; // This wipes the canvas inside
          // Re-add the maximize button and canvas structure for next time
          previewWrapper.innerHTML = `
            <button type="button" class="maximize-btn" title="Maximize Image" aria-label="Maximize">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
        ...
        // 3. Reset UI Elements
        const nE = document.getElementById('image-filename');
    ...
    ['dragenter', 'dragover', 'dragleave', 'drop'].forEach(ev => {
      uploadArea && uploadArea.addEventListener(ev, e => {
        e.preventDefault();
        e.stopPropagation();
      }, false);
    });
    ['dragenter', 'dragover'].forEach(ev => uploadArea && uploadArea.addEventListener(ev, () => uploadArea.classList
      .add('dragover'), false));
    ['dragleave', 'drop'].forEach(ev => uploadArea && uploadArea.addEventListener(ev, () => uploadArea.classList
      .remove('dragover'), false));
    ...
    imageModalOverlay && imageModalOverlay.addEventListener('click', e => {
      if (e.target === imageModalOverlay) imageModalOverlay.classList.remove('visible');
    });

    // Card modal close
    closeCardModalBtn && closeCardModalBtn.addEventListener('click', closeCardModal);
    cardModalOverlay && cardModalOverlay.addEventListener('click', e => {
    ...
    historyList && historyList.addEventListener('click', (e) => {
      ...
      // === UPDATED: Pass overlayPath to displayResults ===
      displayResults(item.result, {
        preview: item.imagePath,
        overlay: item.overlayPath // <--- This triggers the overlay display
      });

      // Handle Main Canvas Image (Standard Logic)
      if (item.imagePath && previewWrapper) {
        ...
        img.onload = () => {
          ...
          rC.height = img.height;
          rC.getContext('2d').drawImage(img, 0, 0);
          const dU = rC.toDataURL();
          previewWrapper.dataset.fullImage = dU;
          const mW = previewWrapper.clientWidth || 900;
          const mH = 400;
          const sc = scaleCanvasToFit(rC, mW, mH);
          displayCanvas(sc, previewWrapper);
          const nE = document.getElementById('image-filename');
          if (nE) {
            nE.textContent = item.filename || 'image';
            nE.style.display = 'block';
          }
        };
      ...
      // Scroll to results
      resultsContainer && resultsContainer.scrollIntoView({
        behavior: 'smooth',
        block: 'start'
      });
    });
    ...
    if (toggleStickyBtn) {
      toggleStickyBtn.addEventListener('click', () => {
        ...
        try {
          localStorage.setItem(STICKY_KEY, wants);
        } catch (e) {
          console.warn('Could not save sticky pref');
        }
      });
    }
    try {
    ...
    if (initialImageSrc && previewWrapper) {
      ...
      img.onload = () => {
        ...
        const nE = $('image-filename');
        if (nE) {
          nE.textContent = (initialPredictData ? (window.__UPLOADED_IMAGE__?.split('/').pop()) : stored
            ?.filename) || 'image';
          nE.style.display = 'block';
        }
        submitBtn && (submitBtn.disabled = false);
      ...
      img.onerror = () => {
        console.warn('Could not load initial image:', initialImageSrc);
        clearState();
      };
