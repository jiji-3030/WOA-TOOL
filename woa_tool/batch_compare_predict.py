...
# radiomics CSV lives in external dataset repo
...
OUT_CSV = os.path.join(BASE_DIR, "results/batch_predictions.csv")
os.makedirs(os.path.dirname(OUT_CSV), exist_ok=True)

# --------------------------------------------------
# CSV fields (WITH SELECTED FEATURES)
# --------------------------------------------------
FIELDNAMES = [
    "image_name",
    "image_path",
    ...
    with open(IMAGE_LIST) as imgs:
        for i, img in enumerate(imgs, 1):
            ...
            if proc.returncode != 0:
                print("⚠️ Skipped (no radiomics match or invalid case)")
            ...
            # --------------------------------------------------
            # WRITE ROW (OPTION B: feature count per prediction)
            # --------------------------------------------------
            row = {
