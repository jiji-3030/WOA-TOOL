...
def predict(model_path: str, image_path: str, tau_override: Optional[float] = None) -> Dict[str, Any]:
    ...
    # 3) Recompute distances/probabilities for display (does not affect deltas/lists)
    ...
    result: Dict[str, Any] = {
        ...
        # Pretty text blocks that match compare_predict’s printout
        "top_features_text": [f"  - {n}: {v:.6f}" for (n, v) in top_mal],
        "all_detected_text": [f"  - {n}: {v:.6f}" for (n, v) in all_pairs],

        # Structured deltas (same data as above, just machine-friendly)
