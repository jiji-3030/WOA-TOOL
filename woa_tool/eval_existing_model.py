# === woa_tool/eval_existing_model.py (enhanced metrics version) ===
...
def _get_class_block(cs, key):
    ...
    for alt in aliases.get(str(key), []):
        if alt in cs:
            return cs[alt]
    raise KeyError(
        f"class_stats missing key compatible with {key!r}; have {list(cs.keys())}"
    )
...
def load_model(model_path):
    ...
    elif "selected_idx" in cfg and cfg["selected_idx"] is not None:
        sel_idx = np.array(cfg["selected_idx"], dtype=int)
    elif cfg.get("selected_names"):
        name_to_idx = {n: i for i, n in enumerate(feature_names)}
        sel_idx = np.array(
            [name_to_idx[n] for n in cfg["selected_names"]], dtype=int
        )
    else:
    ...
    if isinstance(std, dict) and "mu_all" in std and "sig_all" in std:
        ...
        raise KeyError(
            "Model missing normalization stats (standardizer.mu_all/sig_all or global_/train_)."
        )
    ...
    if "Sp_inv" in cfg and cfg["Sp_inv"] is not None:
        ...
        tmp = {
            ...
            "class_stats": cfg.get("class_stats"),
        }
        Sp_inv = build_fallback_Sp_inv(tmp)
        using_fallback = True

    # --- τ ---
    tau = float(cfg.get("tau") or cfg.get("tau_train") or 1.0)
...
def main():
    ap = argparse.ArgumentParser(
        description="Evaluate an existing EWOA/WOA Mahalanobis model on a CSV of images."
    )
    ...
    if labels is not None and np.isfinite(labels).all():
        ...
        sens = tp / max(1, tp + fn)   # Malignant recall
        ...
        print(f"Precision (Positive): {prec:.3f}")
        print(f"Recall/Sensitivity  : {sens:.3f}")
        ...
        # Store summary metrics in CSV (single-row summary)
    ...
    if args.out:
        ...
        if not summary.empty:
            metrics_path = os.path.splitext(args.out)[0] + "_metrics.csv"
            summary.to_csv(metrics_path, index=False)
            print(f"💾 Saved per-image predictions to {args.out}")
            print(f"💾 Saved evaluation metrics summary to {metrics_path}")
        else:
            print(f"💾 Saved per-image predictions to {args.out}")
